...
def get_clean_and_noisy_index(dataset,noise_rate):
    if dataset == 'nuswide21':
        noise = h5py.File('./nus-wide-tc21-lall-noise_{}.h5'.format(noise_rate))
    elif dataset == 'flickr':
        noise = h5py.File('./mirflickr25k-lall-noise_{}.h5'.format(noise_rate))
    elif dataset == 'ms-coco':
        noise = h5py.File('./MSCOCO-lall-noise_{}.h5'.format(noise_rate))
    elif dataset == 'iapr':
        noise = h5py.File('./IAPR-lall-noise_{}.h5'.format(noise_rate))
    fl = list(noise['True'])
...
class DataList(object):
    def __init__(self, dataset, data_type, noise_rate, noise_txt_rate ):
        self.data_type = data_type
        if dataset == 'nuswide21':
            data = loadmat('/data/nus_tc21_clip_split.mat')
            if noise_txt_rate == 0:
                noise_text = None
            else:
                noise_text = loadmat('/data/NUS_noisytext_{}.mat'.format(noise_txt_rate))
            correct_text = torch.load('/data/nus_corrected_txt_features.pt')
            if noise_rate == 0:
                noise = None
            else:
                noise = loadmat('/data/NUS-lall-noise_{}.mat'.format(noise_rate))
        elif dataset == 'flickr':
            data = loadmat('/data/MIRFlickr_clean.mat')
            if noise_txt_rate == 0:
                noise_text = None
            else:
                noise_text = loadmat('/data/MIRFlickr_noisytext_{}.mat'.format(noise_txt_rate))
            correct_text = torch.load('/data/corrected_txt_features.pt')
            if noise_rate == 0:
                noise = None
            else:
                noise = loadmat('/data/mirflickr25k-lall-noise_{}.mat'.format(noise_rate))
        elif dataset == 'ms-coco':
            data = loadmat('/data/coco_clip_split.mat')
            if noise_txt_rate == 0:
                noise_text = None
            else:
                noise_text = loadmat('/data/MSCOCO_noisytext_{}.mat'.format(noise_txt_rate))
            correct_text = torch.load('/data/coco_corrected_txt_features.pt')
            if noise_rate == 0:
                noise = None
            else:
                noise = loadmat('/data/MSCOCO-lall-noise_{}.mat'.format(noise_rate))
        if data_type == "train":
            ...
            if noise_txt_rate == 0:
                ...
                ft = list(noise_text['TagTrain_noisy'])
            fct = correct_text 
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            ...
            lab = lab.astype(int)
            correct_tag = self.correct_tags[index]
            return img, tag, tlab, lab, correct_tag, index
...
def SaveH5File_F(resize_size):
    ...
    query_size = 2000
    root = './data/'
    fi = sio.loadmat(os.path.join(root, 'mirflickr25k-iall-vgg.mat'))['XAll']#image
    fl = sio.loadmat(os.path.join(root, 'mirflickr25k-lall.mat'))['LAll']#label
    ft = sio.loadmat(os.path.join(root, 'mirflickr25k-yall.mat'))['YAll']#text
...
def get_data(config):
    ...
    for data_type in ["train", "test", "database"]:
        dsets[data_type] = DataList(config["dataset"], data_type, config["noise_rate"], config["noise_txt_rate"])
